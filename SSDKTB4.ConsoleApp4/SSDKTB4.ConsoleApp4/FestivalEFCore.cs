...
{
    internal class FestivalEFCore
    {
        private readonly AppModelFirstDbContext _dbcontext;

        public FestivalEFCore()
        {
            _dbcontext = new AppModelFirstDbContext();
        }

        public void Read()
        {
          List<FestivalEntity> festivals= _dbcontext.Festivals.ToList();
			Console.WriteLine("All Festivals (EF Core):");
			Console.WriteLine("-----------------------------------");
			...
			foreach (FestivalEntity item in festivals)
			{
		...
		public void Update()
		{
			...
			var item = _dbcontext.Festivals.Where(x=> x.FestivalId == id).FirstOrDefault();
			if (item is null)
			{
		...
		public void Delete()
		{
			...
			var item = _dbcontext.Festivals.FirstOrDefault(x => x.FestivalId == id);
			...
			_dbcontext.Festivals.Remove(item);
